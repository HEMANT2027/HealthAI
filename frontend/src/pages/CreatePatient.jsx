...
function PatientForm() {
  ...
  // NEW: file-specific handlers with type validation (multiple files)
  const handlePrescriptionFiles = (e) => {
    ...
    if (invalidTypes.length > 0) {
      setError(`Invalid file types: ${invalidTypes.map(f => f.name).join(', ')}. Please upload PDF, JPG, PNG, DOC, or DOCX files`)
      return
    }
    ...
    setPrescriptionFiles(selected)
    setError('')
  }
  
  const handlePathologyFiles = (e) => {
    const selected = Array.from(e.target.files || [])
    if (selected.length === 0) return
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (prescriptionFiles && prescriptionFiles.length > 0) {
        prescriptionFiles.forEach(f => { filesToUpload.push(f); types.push('prescription') })
      }
      if (pathologyFiles && pathologyFiles.length > 0) {
        pathologyFiles.forEach(f => { filesToUpload.push(f); types.push('pathology') })
      }
    ...
    <div className="min-h-screen py-10 px-4 bg-gradient-to-br from-blue-200 via-white to-teal-200 relative">
      <div className="max-w-4xl mx-auto bg-white/90 backdrop-blur-sm border border-gray-200 rounded-3xl shadow-2xl p-6 md:p-10">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-lg text-gray-600">Add Patients and start analyzing their reports.</p>
        </div>
            ...
            <>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Prescription Documents <span className="text-gray-500">(Multiple files: PDF, JPG, PNG, DOC, DOCX)</span>
                </label>
                <input 
                  type="file" 
                  multiple
                  accept=".pdf,.jpg,.jpeg,.png,.doc,.docx"
                  onChange={handlePrescriptionFiles}
                ...
                {prescriptionFiles.length > 0 && (
                  <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded-lg">
                    <p className="text-sm font-medium text-green-800 mb-1">✓ {prescriptionFiles.length} file(s) selected:</p>
                    <ul className="text-sm text-green-700 list-disc list-inside">
                      {prescriptionFiles.map((f,i) => <li key={i}>{f.name}</li>)}
                    </ul>
                  </div>
                )}
              </div>
              ...
              <div className="mt-4">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Pathology Reports <span className="text-gray-500">(Multiple files: PDF, JPG, PNG, DOC, DOCX)</span>
                </label>
                <input 
                  type="file" 
                  multiple
                  accept=".pdf,.jpg,.jpeg,.png,.doc,.docx"
                  onChange={handlePathologyFiles}
                  disabled={isUploading || isSubmitting}
                  className="mt-2 block w-full text-sm text-gray-700 border border-gray-300 rounded-lg p-2 bg-white hover:bg-gray-50 transition-colors cursor-pointer"
                />
                {pathologyFiles.length > 0 && (
                  <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded-lg">
                    <p className="text-sm font-medium text-green-800 mb-1">✓ {pathologyFiles.length} file(s) selected:</p>
                    <ul className="text-sm text-green-700 list-disc list-inside">
                      {pathologyFiles.map((f,i) => <li key={i}>{f.name}</li>)}
                    </ul>
                  </div>
                )}
              </div>
